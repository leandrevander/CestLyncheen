using System.Collections.Generic;
using UnityEngine;

public class BubbleSpawner : MonoBehaviour
{
	
	public List<GameObject> BubbleSpawnerLocation = new List<GameObject>();
	public List<GameObject> AvailableLocation     = new List<GameObject>();

	public float timer;

	public float spawningInterval;

	
	public GameObject BubbleGO;
	// Start is called once before the first execution of Update after the MonoBehaviour is created
	void Start()
	{
		ResetLocation();
	}

	// Update is called once per frame
	void Update()
	{
		timer += Time.deltaTime;
		if (timer >= spawningInterval)
		{
			timer = 0;
			SpawnBubble();
		}
	}

	void SpawnBubble()
	{
		if (AvailableLocation.Count == 0)
		{
			ResetLocation();
		}
		int random =Random.Range(0, AvailableLocation.Count);
		Instantiate(BubbleGO, AvailableLocation[random].transform.position, Quaternion.identity);
		AvailableLocation.RemoveAt(random);
        
	}

	void ResetLocation()
	{
		AvailableLocation = new List<GameObject>(BubbleSpawnerLocation);
	}
}